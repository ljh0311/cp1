...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $parentname = isset($_POST["parentname"]) && !empty($_POST["parentname"]) ? sanitizeInput($_POST["parentname"]) : null;
    $parentmobile = isset($_POST["parentmobile"]) && !empty($_POST["parentmobile"]) ? sanitizeInput($_POST["parentmobile"]) : null;
    ...
    $stmt->bind_result($db_fname, $db_lname, $db_email, $db_username, $db_gender, $db_birthdate, $db_mobile, $db_citizenship, $db_street, $db_code, $db_parentname, $db_parentmobile);
    ...
    if (
        ...
        $street === $db_street &&
        $code === $db_code &&
        $parentname === $db_parentname &&
        $parentmobile === $db_parentmobile
    ) {
    ...
    $stmt = $conn->prepare("UPDATE user_details SET fname=?, lname=?, email=?, username=?, gender=?, birthdate=?, mobile=?, citizenship=?, street=?, code=?, parentname=?, parentmobile=? WHERE userID=?");
    $stmt->bind_param("ssssssssssssi", $fname, $lname, $email, $username, $gender, $birthdate, $mobile, $citizenship, $street, $code, $parentname, $parentmobile, $userID);
