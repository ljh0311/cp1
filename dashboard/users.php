...
<body>
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                ...
                <?php
                    ...
                    if ($conn->connect_error) {
                        ...
                        $stmt = $conn->prepare("SELECT * FROM thetuitioncenter.users");
                    }
                    if (!$stmt->execute()) {
                        ...
                        echo '<table class="table table-bordered table-striped">';
                                ...
                                while($row = $result->fetch_assoc()){
                                    echo "<tr data-email='" . htmlspecialchars($row['email'], ENT_QUOTES) . "'>";
                                        echo "<td>" . htmlspecialchars($row['username']) . "</td>";
                                        echo "<td>" . htmlspecialchars($row['email']) . "</td>";
                                        echo "<td>" . htmlspecialchars($row['password']) . "</td>";
                                        echo "<td>" . htmlspecialchars($row['userTypeId']) . "</td>";
                                        echo "<td>";
                                            echo '<div class="dropdown">';
                                                // Dropdown button
                                                echo '<button class="dropbtn" type="button" id="dropdownMenuButton' . htmlspecialchars($row['email'], ENT_QUOTES) . '" data-bs-toggle="dropdown" aria-expanded="false">';
                                                    echo 'Actions';
                                                echo '</button>';
                                                // Dropdown menu
                                                echo '<div class="dropdown-content" aria-labelledby="dropdownMenuButton' . htmlspecialchars($row['email'], ENT_QUOTES) . '">';
                                                    // View button
                                                    echo '<a class="dropdown-item" href="#" onclick="loadUserDetails(\'' . htmlspecialchars($row['email'], ENT_QUOTES) . '\')" data-bs-toggle="modal" data-bs-target="#userDetailsModal">View</a>';
                                                    // Update button
                                                    echo '<a class="dropdown-item" href="update.php?email=' . htmlspecialchars($row['email'], ENT_QUOTES) . '">Update</a>';
                                                    // Delete button
                                                    echo '<a class="dropdown-item" href="#" onclick="deleteUser(\'' . htmlspecialchars($row['email'], ENT_QUOTES) . '\')">Delete</a>';
                                                echo '</div>';
                                            echo '</div>';
                                        echo "</td>";
                                    echo "</tr>";
    ...
    function loadUserDetails(email) {
        $.ajax({
            url: '/dashboard/fetch_user_details.php', // Update path as needed
            type: 'GET',
            data: {'email': email},
            success: function(data) {
                var userDetails = JSON.parse(data);
                if(userDetails.error) {
                    $('#userDetailsModal .modal-body').html(userDetails.error);
                } else {
                    var detailsHtml = 'Name: ' + userDetails.fname + ' ' + userDetails.lname +
                                    '<br>Email: ' + userDetails.email +
                                    '<br>Gender: ' + userDetails.gender +
                                    '<br>Birthdate: ' + userDetails.birthdate +
                                    '<br>Citizenship: ' + userDetails.citizenship +
                                    '<br>Mobile: ' + userDetails.mobile +
                                    '<br>User Type ID: ' + userDetails.userTypeId;
                    $('#userDetailsModal .modal-body').html(detailsHtml);
                }
            },
            error: function() {
                $('#userDetailsModal .modal-body').html('Error fetching details.');
            }
        });
    }
    ...
    $(document).ready(function() {
        ...
        if(localStorage.getItem('signupSuccess') === 'true') {
            ...
            localStorage.removeItem('signupSuccess'); // Clear the local storage
        }

        // success modal for update 
        if(localStorage.getItem('updateSuccess') === 'true') {
            $('#updateSuccessModal').modal('show');
            localStorage.removeItem('updateSuccess'); // Clear the local storage
        }
        });
